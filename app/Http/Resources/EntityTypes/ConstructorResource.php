...
use App\Services\ConstructorResourceService;
use App\Services\EntityService;
use App\Services\FieldService;
use App\Services\NpaService;

class ConstructorResource extends EntityResource implements EntityResourceContract
{
    /**
    ...
    public function toArray($request)
    {
        ...
        if (!is_null($payload)) {
            ...
            $resourceService = new ConstructorResourceService();
            $npaService      = new NpaService();

            if (!empty($payload->trees)) {
                $treePayloads = $resourceService->getEntityDataPayloads($payload->trees, $this->resource->currentData->user_id);
                $treeNpaIds   = $npaService->getNpaIdsFromPayload($treePayloads);
                $treeNpas     = $npaService->loadNpaLinkData($treeNpaIds);
                
                $payload->trees_data    = $treePayloads;
                $payload->tree_npas     = $treeNpas;
            }
